...
@RegisterTransformClass('der_centraldiff2')
class CentralDiff2(Transform):
    """
    Central difference of order O(h^4)
    Function to numerically estimate first and second derivatives
    ...
    """
    var_list = [('datarate', 10)]

    def func(self, y: array, datarate, n=1):
        # first derivative
        if n == 1:
...
def eotvos(self, lat: array, lon: array, ht: array, derivative: Transform, **kwargs):
    # Constants
    ...
    dlat = derivative(lat, n=1)
    ddlat = derivative(lat, n=2)
    dlon = derivative(lon, n=1)
    ddlon = derivative(lon, n=2)
    dht = derivative(ht, n=1)
    ddht = derivative(ht, n=2)
...
class Eotvos(Transform):
    ...
    var_list = [('derivative', CentralDiff2())]
    func = eotvos
