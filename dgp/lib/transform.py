...
class TransformChain:
    ...
    def addtransform(self, transform):
        if not isinstance(transform, Transform):
            raise TypeError('object must be a subclass of Transform, got an '
                            'object of type {typ}'.format(typ=type(transform)))
    ...
    def __getitem__(self, key):
        uids = self._ordering[key]
        if len(uids) > 1:
            return [self._transforms[uid] for uid in uids]
        else:
